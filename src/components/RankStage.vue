    ...
    class="row row-gap-8 items-end lg:flex-nowrap">
        ...
        <div class="relative">
          <div class="row flex-nowrap items-end justify-between">
            <div class="col-auto mb-[-6%] w-[31%]">
              <NuxtLink :class="`btn btn-light ${props?.list?.find((node:any)=>node?.rank == 2)?.disabled ?'opacity-50 pointer-events-none' :''}`" :href="props?.list?.find((node:any)=>node?.rank == 2)?.href" :target="props?.list?.find((node:any)=>node?.rank == 2)?.target || '_self'">
                <div class="flex justify-center">
                  <div class="relative">
                    <img class="mb-[-4px] ml-[-20px]" src="/assets/img/icon_crown_2.svg" style="width:40px;">
                    <div class="text-center text-[14px] lg:text-[20px]">{{ props?.list?.find((node:any)=>node?.rank == 2)?.name }}</div>
                    <div class="mb-2 text-center text-[12px] lg:text-[18px]" v-html="props?.list?.find((node:any)=>node?.rank == 2)?.number"></div>
                  </div>
                </div>
                <RatioArea ratio="100">
                  <div
                  ...
                  :style="{
                    backgroundImage:`url(${props?.list?.find((node:any)=>node?.rank == 2)?.img})`,
                    filter: 'drop-shadow(0px 0px 16px rgba(255, 255, 255, 0.42))'
                  }"></div>
                </RatioArea>
              </NuxtLink>
            ...
            <div class="col-auto mb-[2%] w-[39%]">
              <NuxtLink :class="`btn btn-light ${props?.list?.find((node:any)=>node?.rank == 1)?.disabled ?'opacity-50 pointer-events-none' :''}`" :href="props?.list?.find((node:any)=>node?.rank == 1)?.href" :target="props?.list?.find((node:any)=>node?.rank == 1)?.target || '_self'">
                <div class="flex justify-center">
                  <div class="relative">
                    <img class="mb-[-4px] ml-[-24px]" src="/assets/img/icon_crown_1.svg" style="width:44px;">
                    <div class="text-center text-[16px] lg:text-[22px]">{{ props?.list?.find((node:any)=>node?.rank == 1)?.name }}</div>
                    <div class="mb-2 text-center text-[13px] lg:text-[20px]" v-html="props?.list?.find((node:any)=>node?.rank == 1)?.number"></div>
                  </div>
                </div>
                <RatioArea ratio="100">
                  <div
                  class="absolute left-0 top-0 size-full rounded bg-cover bg-top"
                  :style="{
                    backgroundImage:`url(${props?.list?.find((node:any)=>node?.rank == 1)?.img})`,
                    filter: 'drop-shadow(0px 0px 16px rgba(255, 255, 255, 0.42))'
                  }"></div>
                </RatioArea>
              </NuxtLink>
            ...
            <div class="col-auto mb-[-10%] w-[31%]">
              <NuxtLink :class="`btn btn-light ${props?.list?.find((node:any)=>node?.rank == 3)?.disabled ?'opacity-50 pointer-events-none' :''}`" :href="props?.list?.find((node:any)=>node?.rank == 3)?.href" :target="props?.list?.find((node:any)=>node?.rank == 3)?.target || '_self'">
                <div class="flex justify-center">
                  <div class="relative">
                    <img class="mb-[-4px] ml-[-16px]" src="/assets/img/icon_crown_3.svg" style="width:32px;">
                    <div class="text-center text-[14px] lg:text-[18px]">{{ props?.list?.find((node:any)=>node?.rank == 3)?.name }}</div>
                    <div class="mb-2 text-center text-[12px] lg:text-[16px]" v-html="props?.list?.find((node:any)=>node?.rank == 3)?.number"></div>
                  </div>
                </div>
                <RatioArea ratio="100">
                  <div
                  class="absolute left-0 top-0 size-full rounded bg-cover bg-top"
                  :style="{
                    backgroundImage:`url(${props?.list?.find((node:any)=>node?.rank == 3)?.img})`,
                    filter: 'drop-shadow(0px 0px 16px rgba(255, 255, 255, 0.42))'
                  }"></div>
                </RatioArea>
              </NuxtLink>
      ...
      :style="{
        ...
        class="relative">
          <div class="absolute left-0 top-0 z-0 size-full rounded" style="background: linear-gradient(#dad9f6 0%, rgba(218, 217, 246, 0.4) 100%); opacity: 0.25;"></div>
          <NuxtLink
          v-for="(node, index) in props?.list?.filter((node:any)=>!([1, 2, 3].includes(Number(node?.rank)))) || []"
          :key="index"
          :href="node?.href"
          :target="props?.list?.[0]?.target || '_self'"
          :class="`btn btn-light relative z-10 block ${node?.disabled === true ?'opacity-50 pointer-events-none' :''}`">
            ...
            <div class="row flex-nowrap items-center">
              <div class="col-auto"><div class="pl-5 _lg:text-[13px]">{{ String(node?.rank || 0).padStart(2, '0') }}</div></div>
              <div class="col-auto">
